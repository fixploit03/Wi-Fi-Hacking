...
Physical Access Attack terhadap WPS Push Button Configuration (PBC) adalah serangan yang memanfaatkan akses fisik ke Access Point untuk menekan tombol WPS dan membuat AP masuk ke mode pairing selama window waktu tertentu (biasanya 2 menit), memungkinkan penyerang yang berada dalam jangkauan fisik untuk menghubungkan perangkat mereka ke jaringan tanpa mengetahui password WPA/WPA2, setelah menekan tombol WPS, Access Point akan menerima koneksi dari client mana pun yang mengirimkan WPS association request dalam window waktu tersebut, sehingga penyerang dapat dengan mudah mendapatkan credential jaringan termasuk SSID dan PSK yang kemudian dapat disimpan dan digunakan untuk akses tidak sah di kemudian hari bahkan setelah window WPS ditutup, serangan ini juga dapat dikombinasikan dengan social engineering di mana penyerang menipu staf atau pengguna yang legitimate untuk menekan tombol WPS dengan alasan teknis tertentu, atau dilakukan di lingkungan publik seperti café, hotel, atau kantor di mana Access Point dapat diakses secara fisik oleh orang yang tidak berwenang, meskipun metode PBC dianggap lebih aman dibandingkan PIN-based WPS karena tidak vulnerable terhadap brute force attack, kelemahan utamanya terletak pada aspek physical security, dan mitigasi yang dapat dilakukan adalah menempatkan Access Point di lokasi yang aman dan tidak mudah diakses oleh unauthorized persons, menonaktifkan WPS sepenuhnya jika tidak diperlukan, mengimplementasikan physical security controls seperti locked cabinets atau restricted access areas, menggunakan WPS dengan timeout window yang sangat singkat, serta melakukan monitoring dan logging terhadap aktivitas WPS untuk mendeteksi unauthorized pairing attempts.
